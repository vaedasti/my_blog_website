		...
		<header id="top">
			<div class="row">
				<div class="header-content twelve columns">
					<h1 id="logo-text"><a href="index.php" title=""><?php echo $website_bilgileri["site_basligi"]; // Websitenin başlığı ?></a></h1>
					<p id="intro"><?php echo $website_bilgileri["site_slogani"]; // Websitenin sloganı ?></p>
				</div>
			</div>
			<nav id="nav-wrap">
				...
				<div class="row">
					<ul id="nav" class="nav">
						<li <?php if ($_SERVER['PHP_SELF'] == "/my_blog_website/index.php" AND empty(htmlspecialchars($_GET['kategoriId']))) echo "class='current'"; // Eğer sayfa index.php ise ve GET yok ise ?>><a href="index.php">Ana Sayfa</a></li>
						<li class="has-children <?php if ($_SERVER['PHP_SELF'] == "/my_blog_website/index.php" AND !empty(htmlspecialchars($_GET['kategoriId']))) echo "current";  // Eğer sayfa index.php ise ve GET var ise ?>"><a href="#">Kategoriler</a>
							<ul>
								<?php
