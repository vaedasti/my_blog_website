<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		if (isset($_POST['menuId']) AND isset($_POST['menuAdi']) AND isset($_POST['menuAdresi'])) {
			sorgu_calistir("UPDATE menu
											SET adi=?, adresi=?
											WHERE id=?",3,array($_POST['menuAdi'], $_POST['menuAdresi'], $_POST['menuId']));
			die();
		}
		elseif (isset($_POST['menuAdi']) AND isset($_POST['menuAdresi'])) {
			sorgu_calistir("INSERT INTO menu(adi, adresi)
											VALUES(?,?)",3,array($_POST['menuAdi'], $_POST['menuAdresi']));
			die();
		}
		elseif (isset($_POST['menuId'])) {
			sorgu_calistir("DELETE FROM menu
											WHERE id=?",3,array($_POST['menuId']));
			die();
		}
	}

	$menuler = sorgu_calistir("SELECT id, adi, adresi
														FROM menu
														ORDER BY id", 2);
?>
<div class="content">
											...
											<tbody>
												...
												<tr role="row" class="odd">
													...
													<td class="td-actions text-right">
														<button type="button" rel="tooltip" class="btn btn-warning btn-icon" data-original-title="Güncelle" onclick="demo.menu(<?php print $menu['id']; ?>, $('input[name=\'<?php print $menu['id']; ?>, adi\']').get()[0].value, $('input[name=\'<?php print $menu['id']; ?>, adresi\']').get()[0].value);">
															<i class="material-icons">cached</i>
															<div class="ripple-container"></div>
														</button>
														<button type="button" rel="tooltip" class="btn btn-danger btn-icon" data-original-title="Sil" onclick="demo.menu(<?php print $menu['id']; ?>);"><i class="material-icons">delete</i><div class="ripple-container"></div></button>
													</td>
												</tr>
												<?php } ?>
											</tbody>
